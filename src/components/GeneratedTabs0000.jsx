...
const syncSectionToDexie = async (sectionIx, nextTabs) => {
  try {
    ...
    const __hasCmp = !!db.table('subValues').schema?.idxByName?.['[ifaceId+secIdx]'];
    const __delQ = __hasCmp
      ? db.table('subValues').where('[ifaceId+secIdx]').equals([ifaceId, Number(sectionIx)])
      : db.table('subValues').where('ifaceId').equals(ifaceId).and(r => Number(r.secIdx) === Number(sectionIx));
    await __delQ.delete();
...
return true;
  ...
  const autoCreateFromValues = useCallback(() => {
    ...
    syncSectionToDexie(Number(secIdx), next).catch(e => console.warn('sync after add failed', e));
...
if (typeof onChange === "function") onChange();
  ...
  const onOpen = useCallback((tab) => {
    ...
    try { try { const idx = tabs.findIndex(t => t.id === tab.id); if (idx !== -1) { const next = tabs.slice(); next[idx] = { ...next[idx], updatedAt: Date.now() }; setTabs(next); writeTabs(next); } } catch {}
 } catch {}
    if (typeof onChange === "function") onChange();
  ...
  // If current section has rows in Dexie but no tabs in UI, hydrate them (e.g., after 'Wstaw wartości podstawowe')
  useEffect(() => {
    (async () => {
      try {
        const ifaceId = String(iface?.id || '');
        if (!ifaceId) return;
        const sIx = Number(activeSec ?? 0);
        const hasInSec = Array.isArray(tabs) && tabs.some(t => Number(t.secIdx) === sIx);
        if (hasInSec) return;
        const rows = await db.table('subValues')
          .where('ifaceId').equals(ifaceId)
          .and(r => Number(r.secIdx) === sIx)
          .toArray();
        if (!rows.length) return;
        const secNo = secNoFor(sIx);
        const fromDexie = rows.map(r => ({
          id: `${ifaceId}_${sIx}_${r.subIdx}`,
          secIdx: sIx,
          secNo,
          snapshot: Array.isArray(r.vals) ? r.vals : []
        }));
        setTabs(prev => Array.isArray(prev) ? [...prev, ...fromDexie] : fromDexie);
      } catch {}
    })();
  }, [iface?.id, activeSec]);
