...
import { db, putSubVals } from '../db/notesDB';
...
const syncSectionToDexie = async (sectionIx, nextTabs) => {
  try {
    ...
    await db.table('subValues').where({ ifaceId, secIdx: Number(sectionIx) }).delete();
...
const secNoFor = (secIdx) => {
  ...
  try {
    if (iface && Array.isArray(iface.sections)) {
...
r(t => Number(t.secIdx) === Number(secIdx));
  const last = inSec[inSec.length - 1];
  if (!last || !Array.isArray(last.snapshot) || last.snapshot.some(v => String(v ?? '') !== '')) {
    const id = Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,7);
    arr.push({ id, secIdx: Number(secIdx), secNo: secNoFor(secIdx), snapshot: emptySnap, updatedAt: 0 });
  }
  return arr;
};
...
const applySnapshot = (tab) => {
  try {
    const id = tab?.id;
    if (!id) return false;
    const idx = Array.isArray(tabs) ? tabs.findIndex(t => t.id === id) : -1;
    if (idx < 0) return false;
    const fieldCount = Array.isArray(iface?.labels) ? iface.labels.length : (Array.isArray(values) ? values.length : 0);
    const snap = normalizeSnapshot(tab?.snapshot, fieldCount);
    const prev = Array.isArray(tabs[idx]?.snapshot) ? tabs[idx].snapshot : [];
    const same = Array.isArray(prev) && Array.isArray(snap) && prev.length === snap.length && prev.every((v,i)=>String(v)===String(snap[i]));
...
// Ensure section ends with an empty tab (blank snapshot). If not, append one.
const ensureTailEmptyTab = (iface, secIdx, list) => {
  const arr = Array.isArray(list) ? list.slice() : [];
  const fieldCount = Array.isArray(iface?.labels) ? iface.labels.length : 0;
  const emptySnap = new Array(fieldCount).fill('');
  const inSec = arr.filter(t => Number(t.secIdx) === Number(secIdx));
  const last = inSec[inSec.length - 1];
  if (!last || !Array.isArray(last.snapshot) || last.snapshot.some(v => String(v ?? '') !== '')) {
    const id = Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,7);
    arr.push({ id, secIdx: Number(secIdx), secNo: secNoFor(secIdx), snapshot: emptySnap, updatedAt: 0 });
...
xt); emitSubIdxChange(idx); } catch {}
  ...
  if (typeof onChange === "function") onChange();
}, [onSwitchSection, tabs, subIdxForTab, activeKey, lastEditedKey, onChange]);
