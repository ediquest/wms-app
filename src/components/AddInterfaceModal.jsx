...
export default function AddInterfaceModal({ open, onClose, categories = [], interfaces = [], onSubmit }) {
  ...
  const idError = useMemo(() => {
    const v = id.trim();
    if (!v) return t('errIdRequired');
    if (!/^[a-z0-9_-]+$/i.test(v)) return t('errIdFormat');
    if (takenIds.has(v)) return t('errIdTaken');
    return '';
  }, [id, takenIds]);
    ...
    <LightModal open={open} onClose={onClose} title={t('newInterfaceTitle')} footer={footer}>
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <label className="form-row" style={{ gridColumn: 'span 1' }}>
          ...
          {categoryError ? <div className="muted" style={{ color: '#f99' }}>{categoryError}</div> : null}
        </label>
