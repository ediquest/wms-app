...
// ---- Helpers to keep IDs unique and ensure trailing empty tab ----
const makeTabId = (ifaceId, secIdx, subIdx) =>
  `${String(ifaceId)}_${Number(secIdx)}_${Number(subIdx)}`;

function normalizeAllTabsForSection(ifaceId, secIdx, tabsList) {
  const sIx = Number(secIdx);
  const keepOthers = (tabsList || []).filter(t => Number(t.secIdx) !== sIx);
  const inSec = (tabsList || [])
    .filter(t => Number(t.secIdx) === sIx)
    .map((t, i) => ({
      ...t,
      secIdx: sIx,
      subIdx: i,
      id: makeTabId(ifaceId, sIx, i),
      secNo: t.secNo ?? secNoFor(sIx),
    }));
  const uniq = [];
  const seen = new Set();
  for (const t of inSec) { if (!seen.has(t.id)) { seen.add(t.id); uniq.push(t); } }
  return [...keepOthers, ...uniq];
}

const isEmptySnapshot = (snap) => !Array.isArray(snap) || snap.every(v => String(v ?? '') === '');

function ensureTailEmptyTab(iface, secIdx, tabsList) {
  const sIx = Number(secIdx);
  const fieldCount = Array.isArray(iface?.labels) ? iface.labels.length : 0;
  const inSec = (tabsList || []).filter(t => Number(t.secIdx) === sIx);
  if (inSec.length === 0) {
    const id = makeTabId(iface.id, sIx, 0);
    return normalizeAllTabsForSection(iface.id, sIx, [...(tabsList || []), {
      id, secIdx: sIx, secNo: secNoFor(sIx), snapshot: new Array(fieldCount).fill('')
    }]);
  }
  const last = inSec[inSec.length - 1];
  if (isEmptySnapshot(last.snapshot)) return tabsList;
  const id = makeTabId(iface.id, sIx, inSec.length);
  const added = [...(tabsList || []), { id, secIdx: sIx, secNo: secNoFor(sIx), snapshot: new Array(fieldCount).fill('') }];
  return normalizeAllTabsForSection(iface.id, sIx, added);
}

function ensureActiveId(currentActiveId, ifaceId, secIdx, tabsList) {
  if ((tabsList || []).some(t => t.id === currentActiveId)) return currentActiveId;
  const firstInSec = (tabsList || []).find(t => Number(t.secIdx) === Number(secIdx));
  return firstInSec ? firstInSec.id : null;
}
...
const syncSectionToDexie = async (sectionIx, nextTabs) => {
  try {
    ...
    const __table = db.table('subValues');
    const __has = !!__table.schema?.idxByName?.['[ifaceId+secIdx]'];
    const __q = __has
      ? __table.where('[ifaceId+secIdx]').equals([ifaceId, Number(sectionIx)])
      : __table.where('ifaceId').equals(ifaceId).and(r => Number(r.secIdx) === Number(sectionIx));
    await __q.delete();
...
useEffect(() => {
  (async () => {
    try {
      const ifaceId = String(iface?.id || '');
      if (!ifaceId) return;
      const rows = await db.table('subValues').where({ ifaceId }).toArray();
...
const addTabForCurrent = useCallback(() => {
  const secIdx = Number(activeSec ?? 0);
  ...
  let next = [...tabs, { id, secIdx, secNo, snapshot }];
  next = normalizeAllTabsForSection(iface.id, secIdx, next);
  next = ensureTailEmptyTab(iface, secIdx, next);
  setTabs(next);
  writeTabs(next);
  setActiveId(id);
  try { syncSectionToDexie(secIdx, next); } catch {}
  try { const idx = subIdxForTab(id, secIdx, next); emitSubIdxChange(idx); } catch {}
  if (typeof onChange === 'function') onChange();
}, [activeSec, tabs, iface, onChange]);
...
return (tu >= bu) ? t : best;
  ...
  const persistActiveSnapshot = useCallback((vals) => {
    try {
      ...
      let __nx = ensureTailEmptyTab(iface, Number(activeSec ?? 0), next);
      setTabs(__nx);
      writeTabs(__nx);
// Keep Dexie in sync with this section
      try { syncSectionToDexie(Number(next[idx]?.secIdx), next); } catch {}
// Mirror snapshot to IndexedDB (Dexie) as subValues
      try {
        const secIdx = Number(next[idx]?.secIdx);
        const secTabs = next.filter(t => Number(t.secIdx) === secIdx);
        const subIdx = secTabs.findIndex(t => t.id === id);
        const fieldCount = Array.isArray(snap) ? snap.length : 0;
        if (iface && secIdx >= 0 && subIdx >= 0) {
          putSubVals(String(iface.id || ''), secIdx, subIdx, Array.isArray(snap) ? snap : []);
        }
      } catch (e) { /* non-fatal */ }
    return true;
    } catch { return false; }
...
const autoCreateFromValues = useCallback(() => {
  // block reentrancy while syncing
  if (isSyncingRef.current) return false;
  const secIdx = Number(activeSec ?? 0);
  const idxs = idxsFor(iface, secIdx);
  if (!idxs.length) return false;
  const hasAny = idxs.some(i => String(values?.[i] ?? '').trim().length > 0);
  if (!hasAny) return false;

  const inSec = (tabs || []).filter(t => Number(t.secIdx) === secIdx);
  let next = tabs;
  if (inSec.length === 0) {
    const secNo = secNoFor(secIdx);
    const id = Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,7);
    const snapshot = (Array.isArray(iface?.labels) ? new Array(iface.labels.length).fill('') : []);
    next = [...tabs, { id, secIdx, secNo, snapshot }];
  }
  // ensure we always have a trailing empty tab in this section
  const normalized = ensureTailEmptyTab(iface, secIdx, next);
  if (normalized.length === (tabs ? tabs.length : 0)) return false; // nothing to do

  setTabs(normalized);
  writeTabs(normalized);
  try { syncSectionToDexie(secIdx, normalized); } catch {}
  if (typeof onChange === 'function') onChange();
  return true;
}, [tabs, values, activeSec, iface, onChange]);
