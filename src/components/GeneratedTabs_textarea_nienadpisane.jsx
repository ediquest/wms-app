...
export default function GeneratedTabs({
  ...
  const mountSkipRef = useRef(true);       // pomiń pierwszy cykl values po mount (wejście z Home)
  ...
  useEffect(() => {
    ...
    if (s !== prevValsStrRef.current) {
      // Pierwszy cykl po mount? Nie traktuj tego jako edycji użytkownika.
      if (mountSkipRef.current) {
        prevValsStrRef.current = s;
        mountSkipRef.current = false;
        console.log("[GT] values-effect -> mount skip (no persist)");
        return;
      }
