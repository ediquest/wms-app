          ...
          {activeSec!==0 && (
            <>
              <div className="labels">
                {(() => {
                  const defSet = new Set((current.defaultFields||[]).map(df => (df.label||'').toLowerCase()));
                  const indicesInSec = current.fieldSections.map((s,i)=> s===activeSec? i : -1).filter(i=>i!==-1);
                  const isDef = (idx) => defSet.has((current.labels[idx]||'').toLowerCase());
                  const ordered = indicesInSec.slice().sort((a,b) => (isDef(b)?1:0) - (isDef(a)?1:0)); // defaults first
                  return ordered.map((fi, k) => (
                    <React.Fragment key={fi}>
                      {(k>0 && isDef(ordered[k-1]) && !isDef(ordered[k])) && <div className="sep-admin strong"></div>}
                      {current.separators.includes(fi) && <div className="sep-admin"></div>}
                      <div className="rowSec" style={{border:'1px dashed var(--border)', padding:'8px', borderRadius:'10px'}}>
                        <input type="text" value={current.labels[fi]} onChange={e => { const n=cloneIface(); n.labels[fi]=e.target.value; applyIface(n); }} />
                        <input type="text" placeholder={t('description')} value={current.descriptions[fi]||''} onChange={e => { const n=cloneIface(); n.descriptions[fi]=e.target.value; applyIface(n); }} />
                        <input type="number" min="1" max="200" value={current.lengths[fi]||10} disabled={!!(current.flexFields && current.flexFields[fi])} style={{opacity: (current.flexFields && current.flexFields[fi]) ? 0.5 : 1}} onChange={e => { const n=cloneIface(); n.lengths[fi]=Math.max(1, Math.min(200, Number(e.target.value)||1)); applyIface(n); }} />
                        <label style={{display:'flex',alignItems:'center',gap:6}}>
                          <input type="checkbox" checked={!!(current.flexFields && current.flexFields[fi])} onChange={e => { const n=cloneIface(); n.flexFields = Array.isArray(n.flexFields) ? n.flexFields.slice() : []; while (n.flexFields.length < n.labels.length) n.flexFields.push(false); n.flexFields[fi] = !!e.target.checked; applyIface(n); }} />
                          <span>Flex</span>
                        </label>
                        <label style={{display:'flex',alignItems:'center',gap:6}}>
                          <input type="checkbox" checked={!!current.required[fi]} onChange={e => { const n=cloneIface(); n.required[fi]=!!e.target.checked; applyIface(n); }} />
                          <span>{t('required')}</span>
                        </label>
                        <select value={current.types[fi]||'alphanumeric'} onChange={e => { const n=cloneIface(); n.types[fi]=(e.target.value==='numeric'?'numeric':'alphanumeric'); applyIface(n); }}>
                          <option value="alphanumeric">{t('alphanumeric')}</option>
                          <option value="numeric">{t('numeric')}</option>
                        </select>
<div className="actions">
                          <button onClick={() => { const n=cloneIface(); if (n.separators.includes(fi)) n.separators=n.separators.filter(x=>x!==fi); else n.separators=n.separators.concat(fi).sort((a,b)=>a-b); applyIface(n); }}>{current.separators.includes(fi) ? t('removeSeparator') : t('addSeparator')}</button>
                          <button onClick={() => { const n=cloneIface(); n.labels.splice(fi,1); n.descriptions.splice(fi,1); n.lengths.splice(fi,1); n.required.splice(fi,1); n.types.splice(fi,1); n.fieldSections.splice(fi,1); n.flexFields = (n.flexFields||[]).slice(); n.flexFields.splice(fi,1); n.separators = n.separators.filter(s => s!==fi).map(s => s>fi? s-1 : s); applyIface(n); const vals = loadValues(); const arr=(vals[currentId]??[]).slice(); arr.splice(fi,1); vals[currentId]=arr; saveValues(vals); }}>{t('delete')}</button>
                        </div>
                      </div>
                    </React.Fragment>
                  ))
